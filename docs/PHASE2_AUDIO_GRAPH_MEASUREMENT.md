...
Measured on 2026-07-18 with the installed Rust 1.85 aarch64 toolchain. The
complete suite passed 330 tests before the final documentation checkpoint.
...
| Soft cubic | Bounded symmetric cubic transfer | Output stays within ±1; third harmonic exceeds 0.02 in the normalized test while second and fourth remain below 0.00001 |
...
functions. Aliasing on high-frequency driven material is therefore a curation
point to listen for and, if objectionable, an `IMPROVE` result rather than an
unmeasured promise.
...
## Raspberry Pi release checkpoint

Pending an on-device release-mode run with the configured graph enabled. Record
the exact JACK rate, frames, periods, xruns, callback count/mean/p95/p99/max,
missed deadlines, oversized callbacks, process/core CPU, RSS, graph-owned
memory, and direct-fallback/client-loss behavior. Use controlled low-gain
captures for unity, bypass transition, chain latency, frequency/dynamics
response, DC, and noise/headroom. Do not hide a deadline failure by changing
JACK settings; record any required change explicitly.
...
Phase 3 delay/modulation work remains blocked until this release checkpoint and
curation sheet are complete.
